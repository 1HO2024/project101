...
.rightmenu ul div {
  ...
  <table>
   <tr>
    ...
    <td rowspan="2">
    <a class="navbar-brand" href="/Company/Main?user_id=${sessionScope.login.user_id}"><img src="/img/로고.png"  alt=회사로고/></a>
      ...
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        ...
        <li class="nav-item dropdown">
          ...
          <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
            <li><a class="dropdown-item" href="/Company/Postlistfilter?user_id=${sessionScope.login.user_id}&compname=${sessionScope.login.compname}">채용공고 목록(조건별)</a></li>
			<li><a class="dropdown-item" href="/Company/Postlistsort?user_id=${sessionScope.login.user_id}&compname=${sessionScope.login.compname}">채용공고 목록(정렬순)</a></li>
          </ul>
        </li>
        ...
        <li class="nav-item dropdown">
          ...
          <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
            <li><a class="dropdown-item" href="/Company/WriteForm?aplnum=${vo.aplnum}&user_id=${sessionScope.login.user_id}&compname=${sessionScope.login.compname}">채용공고 등록</a></li>
            <li><a class="dropdown-item" href="/Company/ListManagement?user_id=${sessionScope.login.user_id}&compname=${sessionScope.login.compname}">등록 공고 관리</a></li>
          </ul>
        </li>
        
        <li class="nav-item">
          <a class="nav-link" href="/Company/ResumeList?user_id=${sessionScope.login.user_id}&compname=${sessionScope.login.compname}">지원 받은 이력서</a>
        </li>
        
        <li class="nav-item">
          <a class="nav-link" href="/Company/Recommend?user_id=${sessionScope.login.user_id}&compname=${sessionScope.login.compname}">인재추천</a>
        </li>
        
        <li class="nav-item" style="font-family: 'Arial Black', 'Impact', sans-serif; font-weight: 900;">
          <a class="nav-link" href="/Company/Bookmark?user_id=${sessionScope.login.user_id}&compname=${sessionScope.login.compname}" style="color: #1A3D91 !important;">PICK ME</a>
        </li>
        
        <li class="nav-item">
          <a class="nav-link" href="/Company/Cslist?user_id=${sessionScope.login.user_id}&compname=${sessionScope.login.compname}">고객센터</a>
        </li>    
      </ul>
    </div>
    </td>
    ...
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
     ...
     <select id="category-select" onchange="filterMessages()">
        ...
        <option value="공고 지원">공고지원</option>
        <option value="인재추천">인재추천</option>
       ...
       <div id="notification-list" style="width:500px;">
             <c:forEach var="message" items="${messagelist}">
                 <div class="message-item" data-category="${message.division}"  style="background-color: white; border-radius:5px; paddin">
                    ...
                    <c:choose>
                <c:when test="${message.division == '공고 지원'}">
                    <a href="/Company/ResumeList?user_id=${message.sender_compid}&compname=${message.sender_compname}">${message.mcontent}</a>
                </c:when>
                <c:when test="${message.division == '인재추천'}">
                    <a href="/Company/Recommend?user_id=${message.sender_compid}&compname=${message.sender_compname}">${message.mcontent}</a>
                </c:when>
                
                <c:when test="${message.division == 'PICK ME'}">
                    <a href="/Company/Bookmarking?user_id=${message.sender_compid}&compname=${message.sender_compname}">${message.mcontent}</a>
                </c:when>  
