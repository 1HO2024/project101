...
<%@ include file="/WEB-INF/views/include/headerindi.jsp" %>
          ...
          <td class="bold" style="text-align: left !important;">
            ...
            <form action="/Individual/Scrap" method="post" style="display: inline-block; margin-left: 5px;">
              ...
              <button type="submit" class="Scrapbtn" style="background-color: transparent; border-color: transparent; display: inline;">
                <svg class="w-10 h-10 text-gray-800 text-dark scrapSvg end-0" xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="${scrapStatus[status.index]}" viewBox="0 0 24 24">
                  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 2l3.09 6.26L22 9.27l-5 4.87L18.18 22 12 18.27 5.82 22 7 14.14 2 9.27l6.91-1.01L12 2z" />
                </svg>
              </button>
            </form>
          </td>
  ...
  $(function() {
    ...
    $('#etc').on('change', function() {
        ...
        let USER_ID = `${sessionScope.login.user_id}`;
        ...
        .done(function(data) {
            ...
            data.sortlist.forEach(function(a) {
               html += "<tr class='job-row' id='job-" + a.aplnum + "'>"
                ...
                + "</td>"
                + "<td style='width:12%;'>"
                + "<a class='btn btn-outline-success' href='/Individual/Postapp?aplnum=" + a.aplnum + "&user_id=" + USER_ID + "'>"
                + "입사지원</a>"
                + "</td>"
                + "</tr>";
            });
...
<div class="pagination">
   <c:if test="${totalPages > 0}">
    <c:if test="${currentPage > 1}">
        <a href="?page=${currentPage - 1}&user_id=${user_id}&sort=${selectedSort}">이전</a>
    </c:if>

    <c:forEach var="i" begin="1" end="${totalPages}">
        <c:choose>
            <c:when test="${i == currentPage}">
                <a href="?page=${i}&user_id=${user_id}&sort=${selectedSort}" class="active">${i}</a> 
            </c:when>
            <c:otherwise>
                <a href="?page=${i}&user_id=${user_id}&sort=${selectedSort}">${i}</a>
            </c:otherwise>
        </c:choose>
    </c:forEach>

    <c:if test="${currentPage < totalPages}">
        <a href="?page=${currentPage + 1}&user_id=${user_id}&sort=${selectedSort}">다음</a>
    </c:if>  
</c:if>
</div>
