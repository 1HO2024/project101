...
<main>
  ...
  <form id="form" action="/Company/Postupdate?aplnum=${param.aplnum}&user_id=${sessionScope.login.user_id}&compname=${sessionScope.login.compname}"  method="POST">
...
<table class="table-container">
    ...
    <tr>
      <td><input class="input-field" type="text" name="post_id" value="${vo.post_id}"></td>
    </tr>
</table>
...
<table class="table-container">
    ...
    <tr>
      <th>모집분야</th>
      <td>
        <%@ include file="/WEB-INF/views/selection/updateDepartment.jsp" %>
      </td>
      <th><span id="red">*</span>모집인원</th>
      <td><input class="input-field" type="number" name="recruitnum" id="recruitnum" value="${vo.recruitnum}" min="0" max="1000"  /></td>
    </tr>
   <tr>
      <th><span id="red">*</span>마감일</th>
      <td colspan="3">
          <input type="datetime-local"     name="deadline"   id="deadline" value="${vo.deadline}" class="deadline" /></td> 
   </tr>
</table>
...
<table class="table-container">
    ...
    <tr>
      <th>근무지역</th>
      <td>
        <%@ include file="/WEB-INF/views/selection/updateWorkspace.jsp" %>
      </td>
      <th><span id="red">*</span>급여</th>
      <td><input class="input-field" type="number"     name="salary"   id="salary" value="${vo.salary}" /></td>
    </tr>
    <tr>
      <th><span id="red">*</span>직무내용</th>
      <td colspan="3" style="width:100%;">
      <textarea class="input-field" name="duty" id = "duty" maxlength="1300" class="textarea2">${vo.duty}</textarea></td>
    </tr>
</table>
...
<table class="table-container">
    ...
    <tr>
      ...
      <td>
        <select class="select-field" name="career" id="career" >
          <option value=""     ${vo.career == '' ? 'selected' : ''}>-선택-</option>
          <option value="무관" ${vo.career == '무관' ? 'selected' : ''}>무관</option>
          <option value="신입" ${vo.career == '신입' ? 'selected' : ''}>신입</option>
          <option value="경력(~2년미만)" ${vo.career == '경력(~2년미만)' ? 'selected' : ''}>경력(~2년미만)</option>
          <option value="경력(2년이상~5년미만)" ${vo.career == '경력(2년이상~5년미만)' ? 'selected' : ''}>경력(2년이상~5년미만)</option>
          <option value="경력(5년이상~10년미만)" ${vo.career == '경력(5년이상~10년미만)' ? 'selected' : ''}>경력(5년이상~10년미만)</option>
          <option value="경력(~10년이상)" ${vo.career == '경력(~10년이상)' ? 'selected' : ''}>경력(~10년이상)</option>
        </select>
      </td>
      <th>경력별 점수</th>
      <td>
    ...
    <tr>
      ...
      <td>
        <select class="select-field" name="edu" id="edu" >
          <option value=""         ${vo.edu == '' ? 'selected' : ''}>-선택-</option>
          <option value="무관"     ${vo.edu == '무관' ? 'selected' : ''}>무관</option>
          <option value="고졸"     ${vo.edu == '고졸' ? 'selected' : ''}>고졸</option>
          <option value="초대졸"   ${vo.edu == '초대졸' ? 'selected' : ''}>초대졸</option>
          <option value="학사이상" ${vo.edu == '학사이상' ? 'selected' : ''}>학사이상</option>
          <option value="석사이상" ${vo.edu == '석사이상' ? 'selected' : ''}>석사이상</option>
          <option value="박사이상" ${vo.edu == '박사이상' ? 'selected' : ''}>박사이상</option>
        </select>
      </td>
      <th>학력별 점수</th>
      <td>
    ...
    <tr>
    <td colspan="4"><input class="input-field" type="text" name="licenses" id="licenses" value="${vo.licenses}" /> </td>
    </tr>
    <tr>
      <th>보유기술</th>
      <td>
    ...
    <tr>
    <td colspan="4"><input class="input-field" type="text" name="skills" id="skills" value="${vo.skills}" /></td>
    </tr>
</table> 
...
<table class="table-container">
    ...
    <tr>
      ...
      <td name="scoreoutput2" id="scoreoutput2">
        <input class="input-field" type="number" name="hopescore" id="hopescore" value="${vo.hopescore}" min="0" max="100" />
      </td>
    </tr>
    <tr>
      <th>설정한 합격점수</th>
...
<table>
	...
	const  goListEl = document.getElementById('goList')
       ...
       location.href = '/Company/Postlistfilter?aplnum=${aplnum}&user_id=${sessionScope.login.user_id}&compname=${sessionScope.login.compname}' 
       }    
